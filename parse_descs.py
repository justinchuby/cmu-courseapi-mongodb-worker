...
def parse_reqs(soup):
    ...
    # Find text
    prereqs = soup.find('dt', string='Prerequisites').parent.parent.dd.string
    coreqs = soup.find('dt', string='Corequisites').parent.parent.dd.string
...
def get_course_desc(num, semester, year):
    ...
    soup = get_page(url)
    if soup is None:
        # Return None if there's an error getting the page
        return None
    ...
    # TODO: potencially clean up the notes
    notes = soup.find('dt', string='Notes').parent.parent.dd.string
